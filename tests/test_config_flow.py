...
async def test_options_flow_scan_interval(hass: HomeAssistant):
    ...
    result = await hass.config_entries.options.async_configure(result["flow_id"], {"scan_interval": 600})
    assert result["type"] is FlowResultType.CREATE_ENTRY
    assert entry.options["scan_interval"] == 600


async def test_options_flow_start_at_meeting_time(hass: HomeAssistant):
    """The meeting-time option is off by default, is stored, and is shown as stored when reopened."""
    import voluptuous_serialize
    from homeassistant.helpers.config_validation import custom_serializer
    ...
    result = await hass.config_entries.options.async_init(entry.entry_id)
    assert _defaults(result) == {"scan_interval": 600, CONF_START_AT_MEETING_TIME: False}
    assert voluptuous_serialize.convert(result["data_schema"], custom_serializer=custom_serializer)

    result = await hass.config_entries.options.async_configure(
        result["flow_id"], {"scan_interval": 600, CONF_START_AT_MEETING_TIME: True}
    )
    assert result["type"] is FlowResultType.CREATE_ENTRY
    assert entry.options == {"scan_interval": 600, CONF_START_AT_MEETING_TIME: True}

    result = await hass.config_entries.options.async_init(entry.entry_id)
    assert _defaults(result)[CONF_START_AT_MEETING_TIME] is True
