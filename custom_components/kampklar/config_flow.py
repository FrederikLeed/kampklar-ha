...
class KampKlarConfigFlow(ConfigFlow, domain=DOMAIN):
    ...
    @staticmethod
    @callback
    def async_get_options_flow(config_entry: ConfigEntry) -> KampKlarOptionsFlow:
        """Return the options flow."""
        return KampKlarOptionsFlow()
...
class KampKlarOptionsFlow(OptionsFlow):
    """Options: polling interval and when calendar events start."""

    async def async_step_init(self, user_input: dict[str, Any] | None = None) -> ConfigFlowResult:
        """Manage the poll interval and calendar options."""
        if user_input is not None:
        ...
        schema = vol.Schema(
            {
                ...
                vol.Required(
                    CONF_START_AT_MEETING_TIME,
                    default=options.get(CONF_START_AT_MEETING_TIME, DEFAULT_START_AT_MEETING_TIME),
                ): BooleanSelector(),
            }
        )
